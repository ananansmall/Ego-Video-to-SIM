"""轨迹加载器 — 复用 002_render_scene.py 的 HaWoR+GLB 轨迹加载逻辑
...
def _load_glb_sapien(glb_path, scale, R_inv, t_inv, scene, logger=None):
    """SAPIEN: 加载 GLB 物体 (对齐 002_render_scene.py)
    ...
    try:
        ...
        # s_inv = 1/scale (对齐 002_render_scene.py: 用逆缩放将 RAS 米制 → HaWoR 米制)
        ...
        for idx, (name, mesh) in enumerate(mesh_dict.items()):
            ...
            # 变换链 (对齐 002_render_scene.py L917-918):
            ...
            # 关键: 顶点已在世界坐标系, pose 设为原点 (对齐 002_render_scene.py L950)
...
def _load_glb_pybullet(glb_path, scale, R_inv, t_inv, physics_client, logger=None):
    """PyBullet: 加载 GLB 物体 (对齐 002_render_scene.py, 用 s_inv=1/scale)"""
    try:
        import trimesh
        ...
        for name, mesh in mesh_dict.items():
            ...
            # 变换链 (对齐 002_render_scene.py): s_inv * R_inv @ p + t_inv → RXWORLD_TO_SAPIEN @ p
            vertices_hawor = s_inv * (R_inv @ vertices.T).T + t_inv
            vertices_sapien = (RXWORLD_TO_SAPIEN @ vertices_hawor.T).T
