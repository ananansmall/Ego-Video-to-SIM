...
from galaxea_sim.utils import torch_utils
...
class Kinematics:
    def __init__(
        self,
        urdf_path: str,
        end_link: sapien.pysapien.physx.PhysxArticulationLinkComponent,
        articulation: sapien.pysapien.physx.PhysxArticulation,
        controlled_active_joint_indices: list,
        device: str = "cpu",
    ):
        """
        ...
        self.controlled_active_joint_indices = controlled_active_joint_indices
        ...
        active_ancestor_joints: List[sapien.pysapien.physx.PhysxArticulationJoint] = []
        while cur_link is not None:
            if cur_link.joint in self.articulation.active_joints:
        ...
        self.active_ancestor_joint_idxs = [
            self.articulation.active_joints.index(x) for x in self.active_ancestor_joints
        ]
        self.controlled_joints_idx_in_qmask = [
            self.active_ancestor_joint_idxs.index(idx)
            for idx in self.controlled_active_joint_indices
        ]
    ...
    def _setup_cpu(self):
        ...
        self.qmask = torch.zeros(
            self.articulation.dof, dtype=torch.bool, device=self.device
        )
        self.pmodel: PinocchioModel = self.articulation.create_pinocchio_model()
        self.qmask[self.controlled_active_joint_indices] = 1
    ...
    def compute_ik(
        self,
        target_pose: sapien.pysapien.Pose,
        q0: torch.Tensor,
        pos_only: bool = False,
        action=None,
